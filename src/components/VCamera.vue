...
watchEffect(() => {
  ...
  state.currentVideo = new File(videoBlob.value, state.currentVideoName as string, {
    type: "video/webm",
  });
  watchEffect(() => {
...
<template>
  <VBtn icon @click="enabled = !enabled" v-if="!enabled" class="br text-white bg-primary fixed m-4 hover:bg-secondary scale"
    title="Record Video"
  >
